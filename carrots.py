...
RULE_TEXT = ['ПРАВИЛА:', 'Собирайте овощи, чтобы пройти на следующий уровень',
             'Не роняйте овощи! Иначе вы будете терять жизнь']
...
def game_over(cur_score):
    ...
    elif LEVEL > len(SCORE_CONST):
        txt = [f'Вы прошли все {LEVEL - 1} существующих уровней.', f' Ждите обновлений!', f'Ваш счёт: {cur_score}',
               f'Рекорд: {record_score} был установлен: {date}']
...
class Player(pygame.sprite.Sprite):
    def __init__(self, columns, rows):
        ...
        self.cut_sheet(0, sheets[0], 10, 1)
        self.cut_sheet(1, sheets[1], 8, 1)
        self.cut_sheet(2, sheets[2], 8, 1)
        self.cur_frame = 0
        self.image = self.frames[self.sheet_id][self.cur_frame]
        self.rect = self.rect.move(width / 2, height - 100)

    def cut_sheet(self, ind, sheet, columns, rows):
        self.rect = pygame.Rect(0, 0, sheet.get_width() // columns,
                                sheet.get_height() // rows)
        for j in range(rows):
            for i in range(columns):
                frame_location = (self.rect.w * i, self.rect.h * j)
                self.frames[ind].append(sheet.subsurface(pygame.Rect(
                    frame_location, self.rect.size)))

    def update(self):
        self.speedx = 0
        if not PAUSED:
            self.cur_frame = (self.cur_frame + 1) % len(self.frames[self.sheet_id])
            self.image = self.frames[self.sheet_id][self.cur_frame]
...
if __name__ == '__main__':
    ...
    s1 = pygame.mixer.Sound('sounds//pause_music.mp3')
    s2= pygame.mixer.Sound('sounds//pick_up_music.mp3')
    s3 = pygame.mixer.Sound('sounds//new_level_music.mp3')
    s4 = pygame.mixer.Sound('sounds//game_over.mp3')
